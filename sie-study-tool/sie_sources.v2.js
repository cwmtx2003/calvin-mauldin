// ═══════════════════════════════════════════════════════════════════
// SIE_SOURCES — OFFLINE build manifest (de-Kaplan overhaul).
// NOT shipped to the browser; NOT script-tagged in index.html.
// Built by build_sources.py from the FINRA SIE Content Outline + the
// public-domain corpus. SRO refs (ship:false) are read from corpus/ at
// generation time only and never embedded. No Kaplan material.
// ═══════════════════════════════════════════════════════════════════
window.SIE_SOURCES = {
 "version": "2.0.0",
 "generated_at": "2026-06-14",
 "license": {
  "generated_content": "CC BY-NC-SA 4.0",
  "commercial_use": false
 },
 "source_registry": {
  "FINRA": {
   "type": "sro",
   "name": "FINRA Rulebook",
   "path": "corpus/FINRA_Rules_and_ByLaws.zip",
   "ship": false,
   "license": "SRO",
   "present": true
  },
  "MSRB": {
   "type": "sro",
   "name": "MSRB Rulebook",
   "path": "corpus/MSRB-Rule-Book-Current-Version.txt",
   "ship": false,
   "license": "SRO",
   "present": true
  },
  "SA33": {
   "type": "statute",
   "name": "Securities Act of 1933",
   "path": "corpus/Securities_Act_of_1933.txt",
   "ship": true,
   "license": "public-domain",
   "present": true
  },
  "17_CFR_Part_230": {
   "type": "cfr",
   "name": "17 CFR Part 230",
   "path": "corpus/17 CFR Part 230 (up to date as of 5-29-2026).txt",
   "ship": true,
   "license": "public-domain",
   "present": true
  },
  "SEA34": {
   "type": "statute",
   "name": "Securities Exchange Act of 1934",
   "path": "corpus/sea34.txt",
   "ship": true,
   "license": "public-domain",
   "present": true
  },
  "CBOE": {
   "type": "sro",
   "name": "CBOE C1 Rulebook",
   "path": "corpus/C1_Exchange_Rule_Book.txt",
   "ship": false,
   "license": "SRO",
   "present": true
  },
  "17_CFR_Part_240": {
   "type": "cfr",
   "name": "17 CFR Part 240",
   "path": "corpus/17 CFR Part 240 (up to date as of 5-29-2026).txt",
   "ship": true,
   "license": "public-domain",
   "present": true
  },
  "ICA40": {
   "type": "statute",
   "name": "Investment Company Act of 1940",
   "path": "corpus/Investment Company Act of 1940.txt",
   "ship": true,
   "license": "public-domain",
   "present": true
  },
  "17_CFR_Part_270": {
   "type": "cfr",
   "name": "17 CFR Part 270",
   "path": "corpus/17 CFR Part 270 (up to date as of 5-29-2026).txt",
   "ship": true,
   "license": "public-domain",
   "present": true
  }
 },
 "outline": {
  "full": "Securities Industry Essentials (SIE) \nExamination \n\n \n\nC O N T E N T  O U T L I N E  \n\n© 2024 FINRA \n \n\n\n\n  \n\nPURPOSE OF THE EXAM \nThe Securities Industry Essentials (SIE) exam assesses a candidate’s basic knowledge of the securities industry. The exam \nfocuses on industry terminology, securities products, the structure and function of the markets, regulatory agencies and their \nfunctions, and regulated and prohibited practices. For more information about the SIE, see FINRA Rule 1210.03. \n \nSTRUCTURE OF THE EXAM \nThe exam consists of 75 multiple-choice items, and each item consists of four answer choices. The allocation of test items is \nas follows: \n \n\nPercentage of \nSection Description Number of Items \n\nExam Items \n1 Knowledge of Capital Markets 16% 12 \n2 Understanding Products and Their Risks 44% 33 \n\nUnderstanding Trading, Customer Accounts and \n3 31% 23 \n\nProhibited Activities \n4 Overview of the Regulatory Framework 9% 7 \n\nTOTAL 100% 75 \n \nMuch of the content on the SIE outline is based on common securities industry knowledge and general concepts. This \nknowledge is often not based on any one rule or regulation. Examples include basic characteristics of investment products \nsuch as the relationship between the price and yield of a bond and the rights of a common stockholder. Alternately, some \nknowledge is based on specific rules and regulations. Examples include the reporting requirements for a Suspicious Activity \nReport (SAR) and the Uniform Application for Securities Industry Registration or Transfer (Form U4) filing requirements. The \nexam will assess candidates on both rule- and non-rule-based industry knowledge. \n \nSeveral federal laws and regulations serve as the foundation for some of the knowledge contained in this outline. The content \nupon which the outline is based includes but is not limited to rules and regulations under the following Acts: \n\n \nThe Securities Act of 1933 \nThe Securities Exchange Act of 1934 \nThe Investment Company Act of 1940 \nThe Investment Adviser Act of 1940 \n\n \nADMINISTRATION OF THE EXAM \nThe exam is administered via computer. A tutorial on how to take the exam is provided prior to taking the exam. Each \ncandidate’s exam includes 10 additional, unidentified pretest items that do not contribute toward the candidate's score. The \npretest items are randomly distributed throughout the exam. Therefore, each candidate’s exam consists of a total of 85 items \n(75 scored and 10 unscored). There is no penalty for guessing. Therefore, candidates should attempt to answer all items. \nCandidates will be allowed 1 hour and 45 minutes to complete the SIE exam. \n \nAll candidate test scores are placed on a common scale using a statistical adjustment process known as equating. Equating \nscores to a common scale accounts for the slight variations in difficulty that may exist among the different sets of exam items \nthat candidates receive. This allows for a fair comparison of scores and ensures that every candidate is held to the same \npassing standard regardless of which set of exam items they received.  \n \nCandidates are not permitted to bring reference materials to their testing session. Severe penalties are imposed on candidates \nwho cheat or attempt to cheat on FINRA-administered exams. \n \nPlease visit www.finra.org for additional details.  \n\n2 \n\n\n\n  \n\nSection 1: Knowledge of Capital Markets \n \n1.1 Regulatory Entities, Agencies and Market Participants \n \n1.1.1  The Securities and Exchange Commission (SEC) \n\n• The high-level purpose and mission of securities regulation \n• Definition, jurisdiction and authority of the SEC \n\n \n1.1.2 Self-regulatory Organizations (SROs) \n\n• Purpose and mission of an SRO \n• Jurisdiction and authority of SROs (e.g., CBOE, FINRA, MSRB) \n\n \n1.1.3 Other Regulators and Agencies \n\n• Department of the Treasury/IRS \n• State regulators (e.g., NASAA) \n• The Federal Reserve \n• Securities Investor Protection Corporation (SIPC) \n• Federal Deposit Insurance Corporation (FDIC) \n\n \n1.1.4 Market Participants and their Roles \n\n• Investors (e.g., accredited, institutional, retail) \n• Broker-Dealers (e.g., introducing, clearing, prime brokers) \n• Investment advisers \n• Municipal advisors \n• Issuers and underwriters \n• Traders and market makers \n• Custodians and trustees \n• Transfer agents \n• Depositories and clearing corporations (e.g., Depository Trust & Clearing Corporation (DTCC), Options Clearing \n\nCorporation (OCC)) \n \n1.2 Market Structure \n \n1.2.1 Types of Markets \n\n• The primary market \n• The secondary market (e.g., electronic, over-the-counter (OTC), physical) \n• The third market \n• The fourth market \n\n \n1.3 Economic Factors \n \n1.3.1 The Federal Reserve Board’s Impact on Business Activity and Market Stability \n\n• Monetary vs. fiscal policy \n• Open market activities and impact on economy \n• Different rates (e.g., interest rate, discount rate, federal funds rate) \n\n \n1.3.2 Business Economic Factors \n\n• Purpose of financial statements (e.g., balance sheet, income statement) \n• Business cycle (e.g., contraction, trough, expansion, peak) \n• Indicators (e.g., leading, lagging, coincident, inflation) \n• Basic effects on bond and equity markets (e.g., cyclical, defensive, growth) \n\n3 \n\n\n\n  \n• Principal economic theories (e.g., Keynesian, Monetarist) \n\n \n1.3.3  International Economic Factors \n\n• U.S. balance of payments \n• Gross domestic product (GDP), gross national product (GNP) \n• Exchange rates \n\n \n1.4 Offerings \n \n\n• Roles of participants (e.g., investment bankers, underwriting syndicate, municipal advisors) \n• Types of offerings \n\n Public vs. private securities offering \n Initial public offering (IPO), secondary offering and follow-on offering \n Methods of distribution (e.g., best efforts, firm commitment) \n\n• Shelf registrations and distributions (e.g., definition, purpose) \n• Types and purpose of offering documents and delivery requirements (e.g., official statement, program disclosure \n\ndocument, prospectus) \n• Regulatory filing requirements and exemptions (e.g., SEC, blue-sky laws) \n\n \nRules \n \nFINRA Rules \n\n2266 – SIPC Information \n2269 – Disclosure of Participation or Interest in Primary or Secondary Distribution \n5250 – Payments for Market Making \n\n \nMSRB Rules \n\nG-11 – Primary Offering Practices \nG-32 – Disclosures in Connection with Primary Offerings \nG-34 – CUSIP Numbers, New Issue and Market Information Requirements  \n\n \nSEC Rules and Regulations \n\nSecurities Act of 1933 \nSection 7 – Information Required in a Registration Statement \nSection 8 – Taking Effect of Registration Statements and Amendments Thereto \nSection 10 – Information Required in Prospectus \nSection 23 – Unlawful Representations \n215 – Accredited Investor \n431 – Summary Prospectuses \nSchedule A – Schedule of Information Required in Registration Statement \nSchedule B – Schedule of Information Required in Registration Statement \n\nSecurities Exchange Act of 1934 \nSection 3(a) – Definitions and Application of Title \nSection 12 — Registration Requirements for Securities \nSection 15 – Registration and Regulation of Brokers and Dealers \nSection 15A – Registered Securities Associations \nRegulation D – Rules Governing the Limited Offer and Sale of Securities Without Registration Under the \nSecurities Act of 1933 \n\n144 – Persons Deemed Not to Be Engaged in a Distribution and Therefore Not Underwriters  \n144A – Private Resales of Securities to Institutions \n145 – Reclassification of Securities, Mergers, Consolidations and Acquisitions of Assets \n147 – \"Part of an Issue,\" \"Person Resident,\" and \"Doing Business Within\" for Purposes of Section 3(a)(11) \n164 – Post-filing Free Writing Prospectuses in Connection with Certain Registered Offerings \n\n4 \n\n\n\n  \n \nSecurities Investor Protection Act of 1970 (SIPA)  \n\n5 \n\n\n\n  \n\nSection 2: Understanding Products and Their Risks \n \n2.1 Products \n \n2.1.1  Equity Securities \n\n• Types of equities \n Common stock \n Preferred stock \n Rights \n Warrants \n American Depositary Receipts (ADRs) \n\n \nKnowledge of: \n\n• Ownership (e.g., order of liquidation, limited liability) \n• Voting rights \n• Convertible \n• Control and restrictions (e.g., SEC Rule 144) \n\n \n2.1.2  Debt Instruments \n\n• Treasury securities (e.g., bills, notes, receipts, bonds) \n• Agency (e.g., asset-backed and mortgage-backed securities) \n• Corporate bonds \n• Municipal securities \n\n General obligation (GO) bonds \n Revenue bonds \n Others (e.g., special type bonds, taxable municipal securities, short-term obligations) \n\n• Others (e.g., money market instruments, certificate of deposit (CD), bankers’ acceptance, commercial paper) \n \n\nKnowledge of: \n• Varying maturities \n• Generate income (e.g., interest) \n• Coupon value \n• Par value \n• Yield \n• Ratings and rating agencies \n• Callable and convertible features \n• Short-term vs. long-term characteristics \n• Relationship between price and interest rate \n• Negotiated vs. competitive offerings via underwriters and syndicates \n• Auction \n\n \n2.1.3  Options \n\n• Types of options \n Puts and calls \n Equity vs. index \n\n \nKnowledge of: \n\n• Hedging or speculation \n• Expiration date \n• Strike price \n• Premium \n\n6 \n\n\n\n  \n• Underlying or cash settlement \n• In-the-money, out-of-the money \n• Covered vs. uncovered \n• American vs. European \n• Exercise and assignment  \n• Varying strategies (e.g., long, short) \n• Special disclosures (e.g., Options Disclosure Document (ODD)) \n• Options Clearing Corporation (OCC) for listed options \n\n \n2.1.4  Packaged Products \n\n• Investment companies \n Types of investment companies \n\n− Closed-end funds \n− Open-end funds \n− Unit investment trusts (UITs) \n− Variable contracts/annuities \n\n \nKnowledge of: \n\n• Loads \n• Share classes \n• Net asset value (NAV) \n• Disclosures \n• Costs and fees \n• Breakpoints \n• Right of accumulation (ROA) \n• Letter of intent (LOI) \n• Net transactions \n• Surrender charges \n• Sales charges \n\n \n2.1.5 Municipal Fund Securities \n\n• 529 Plans \n Prepaid tuition \n Savings plans \n\n• Local government investment pools (LGIPs) \n• ABLE accounts \n\n \nKnowledge of: \n\n• Municipal fund securities \n• Owner vs. beneficiary \n• Restricted use of plan assets \n• Tax advantages \n• Direct or adviser sold \n\n \n2.1.6  Direct Participation Programs (DPPs) \n\n• Types of DPPs \n Limited partnerships \n Tenants in common (TIC) \n\n \n  \n\n7 \n\n\n\n  \nKnowledge of: \n\n• Pass-through tax treatment \n• Unlisted \n• Generally illiquid \n\n \n2.1.7  Real Estate Investment Trusts (REITs) \n\n• Types of REITs \n Private \n Registered, non-listed \n Listed \n\n \nKnowledge of: \n\n• Real estate equity or debt \n• Tax-advantaged income without double taxation \n\n \n2.1.8  Hedge Funds \n \n\nKnowledge of: \n• Minimum investment \n• Partnership structure \n• Private equity \n• Generally illiquid \n\n \n2.1.9  Exchange-traded Products (ETPs) \n\n• Types of ETPs \n Exchange-traded funds (ETFs) \n Exchange-traded notes (ETNs) \n\n \nKnowledge of: \n\n• Alternative investments to mutual funds \n• Fee considerations \n• Active vs. passive \n\n \n2.2 Investment Risks \n \n\n• Definition and Identification of Risk Types \n Capital \n Credit \n Currency \n Inflationary/purchasing power \n Interest rate/reinvestment \n Liquidity \n Market/systematic \n Non-systematic \n Political \n Prepayment \n\n \n• Strategies for Mitigation of Risk \n\n Diversification \n Portfolio rebalancing \n Hedging \n\n8 \n\n\n\n  \nRules  \n\n \nFINRA Rules \n\n2261 – Disclosure of Financial Condition \n2262 – Disclosure of Financial Relationship with Issuer \n2310 – Direct Participation Programs \n2330 – Members’ Responsibilities Regarding Deferred Variable Annuities  \n2342 – “Breakpoint” Sales \n2360 – Options \n\n \nMSRB Rules \n\nD-12 – Definition of Municipal Fund Securities \nG-17 – Conduct of Municipal Securities and Municipal Advisory Activities \nG-30 – Pricing and Commissions \nG-45 – Reporting of Information on Municipal Fund Securities \n\n \nCBOE Rule \n\nRule 1.1 – Definitions \n \n\nSEC Rules and Regulations \nSecurities Exchange Act of 1934 \n\n3a11-1 – Definition of the Term \"Equity Security\" \n10b-18 – Purchases of Certain Equity Securities by the Issuer and Others \n\nInvestment Company Act of 1940  \nSection 3(a) – Definitions - “Investment Company” \nSection 4 – Classification of Investment Companies \nSection 5 – Subclassification of Management Companies \n12b-1 – Distribution of Shares by Registered Open-end Management Investment Company \n\n  \n\n9 \n\n\n\n  \n\nSection 3: Understanding Trading, Customer Accounts and Prohibited Activities \n \n3.1 Trading, Settlement and Corporate Actions \n \n3.1.1  Orders and Strategies \n\n• Types of orders (e.g., market, stop, limit, good-til-canceled (GTC), discretionary vs. non-discretionary, solicited \nvs. unsolicited) \n\n• Buy and sell, bid-ask \n• Trade capacity (e.g., principal, agency)  \n• Long and short, naked and covered \n• Bearish and bullish \n\n \n3.1.2  Investment Returns \n\n• Components of return (e.g., interest, dividends, realized/unrealized gains, return on capital)  \n• Different types of dividends (e.g., cash, stock) \n• Dividend payment dates (e.g., record date, ex-dividend date, payable date)  \n• Concepts of measurement (e.g., yield, yield to maturity (YTM), yield to call (YTC), total return, basis points) \n• Cost basis requirements \n• Benchmarks and indices \n \n\n3.1.3  Trade Settlement  \n• Settlement time frames for various products (e.g., T, T + 1) \n• Physical vs. book entry (e.g., delivery and settlement) \n\n \n3.1.4  Corporate Actions \n\n• Types of corporate actions (e.g., splits, reverse splits, buybacks, tender offers, exchange offers, rights offerings, \nmergers and acquisitions (M&A)) \n\n• Impact of stock splits and reverse stock splits on market price and cost basis \n• Adjustments to securities subject to corporate actions \n• Delivery of notices and corporate action deadlines \n• Proxies and proxy voting \n\n \n3.2 Customer Accounts and Compliance Considerations \n\n \n3.2.1  Account Types and Characteristics \n\n• Cash \n• Margin \n• Options \n• Discretionary vs. non-discretionary \n• Fee-based vs. commission \n• Educational accounts \n\n \n3.2.2  Customer Account Registrations \n\n• Individual \n• Joint \n• Corporate/institutional \n• Trust (e.g., revocable, irrevocable) \n• Custodial (e.g., UTMA) \n• Partnerships \n• Retirement (e.g., individual retirement account (IRA), qualified plans) \n\n◦ Types and characteristics \n\n10 \n\n\n\n  \n◦ Required minimum distributions \n◦ Contributions \n\n \n3.2.3 Anti-money Laundering (AML) \n\n• Definition of money laundering \n• Stages of money laundering (e.g., structuring, layering, placement) \n• AML compliance program \n• Suspicious Activity Report (SAR) \n• Currency Transaction Report (CTR) \n• FinCEN \n• Office of Foreign Asset Control (OFAC) and the Specially Designated Nationals and Blocked Persons (SDNs) \n\nList \n \n3.2.4  Books and Records and Privacy Requirements \n\n• Books and records retention requirements  \n• Confirmations and account statements \n• Holding of customer mail \n• Business continuity plans (BCP) \n• Customer protection and custody of assets \n• Privacy requirements (e.g., Regulation S-P) \n\n◦ Nonpublic personal information \n◦ Confidentiality of information \n◦ Privacy notifications \n◦ Safeguard requirements \n\n \n3.2.5  Communications with the Public and General Best Interest Obligations and Suitability Requirements \n\n• Communications with the public and telemarketing \n◦ Classifications and general requirements \n◦ Do-not-call list \n\n• Best interest obligations and suitability requirements \n◦ Know-your-customer (KYC) \n◦ General requirements (e.g., what constitutes a recommendation) \n\n \n3.3 Prohibited Activities \n\n \n3.3.1  Market Manipulation \n\n• Definition of market manipulation \n• Types of market manipulation (e.g., market rumors, pump and dump, front running, excessive trading, marking \n\nthe close, marking the open, backing away, freeriding) \n \n3.3.2  Insider Trading \n\n• Definition of insider trading  \n• Definition of material nonpublic information \n• Identifying involved parties \n• Penalties (e.g., fines, expulsion, incarceration) \n\n \n3.3.3  Other Prohibited Activities \n\n• Restrictions preventing associated persons from purchasing initial public offerings (IPOs) \n• Use of manipulative, deceptive or other fraudulent devices \n• Improper use of customers’ securities or funds \n\n◦ Borrowing from customers \n◦ Sharing in customer accounts \n\n11 \n\n\n\n  \n• Financial exploitation of seniors \n• Activities of unregistered persons \n\n◦ Prohibition against paying commissions to unregistered persons \n◦ Prohibition against solicitation of customers and taking orders \n\n• Falsifying or withholding documents \n◦ Signatures of convenience \n◦ Responding to regulatory requests \n\n• Prohibited activities related to maintenance of books and records (e.g., falsifying records and improper \nmaintenance/retention of records) \n\n \nRules \n\n \nFINRA Rules \n\n2010 – Standards of Commercial Honor and Principles of Trade \n2020 – Use of Manipulative, Deceptive or Other Fraudulent Devices \n2040 – Payments to Unregistered Persons \n2090 – Know Your Customer \n2111 – Suitability \n2120 – Commissions, Mark Ups and Charges  \n2150 – Improper Use of Customers’ Securities or Funds; Prohibition Against Guarantees and Sharing in Accounts \n2165 – Financial Exploitation of Specified Adults \n2210 – Communications with the Public \n2231 – Customer Account Statements  \n2251 – Forwarding of Proxy and Other Issuer-related Materials  \n2264 – Margin Disclosure Statement \n2232 – Customer Confirmations \n3150 – Holding of Customer Mail \n3210 – Accounts at Other Broker-Dealers and Financial Institutions \n3230 – Telemarketing \n3240 – Borrowing from or Lending to Customers \n3250 – Designation of Accounts  \n3260 – Discretionary Accounts  \n3310 – Anti-money Laundering Compliance Program \n4210 – Margin Requirements \n4370 – Business Continuity Plans and Emergency Contact Information \n4511 – General Requirements \n4512 – Customer Account Information \n4514 – Authorization Records for Negotiable Instruments Drawn From a Customer's Account \n5130 – Restrictions on the Purchase and Sale of Initial Equity Public Offerings \n5210 – Publication of Transactions and Quotations \n5220 – Offers at Stated Prices \n5230 – Payments Involving Publications that Influence the Market Price of a Security \n5240 – Anti-intimidation/Coordination \n5270 – Front Running of Block Transactions \n5280 – Trading Ahead of Research Reports \n5290 – Order Entry and Execution Practices  \n5310 – Best Execution and Interpositioning \n5320 – Prohibition Against Trading Ahead of Customer Orders \n6438 – Displaying Priced Quotations in Multiple Quotation Mediums  \n \n\nMSRB Rules \nG-8 – Books and Records to be Made by Brokers, Dealers, Municipal Securities Dealers, and Municipal Advisors \n\n12 \n\n\n\n  \nG-9 – Preservation of Records \nG-13 – Quotations \nG-14 – Reports of Sales or Purchases \nG-15 – Confirmation, Clearance, Settlement and Other Uniform Practice Requirements with Respect to Transactions \nwith Customers \nG-18 – Best Execution \nG-21 – Advertising \nG-25 – Improper Use of Assets \nG-39 – Telemarketing \nG-41 – Anti-money Laundering Compliance Program \nG-47 – Time of Trade Disclosure \n\n \nSEC Rules and Regulations \n\nRegulation M \nRegulation S-P – Privacy of Consumer Financial Information and Safeguarding Personal Information \nSecurities Exchange Act of 1934 \n\nSection 10 – Regulation of the Use of Manipulative and Deceptive Devices \nSection 11(d) – Trading by Members of Exchanges, Brokers and Dealers – “Prohibition on Extension of Credit by \nBroker-Dealer” \nSection 14 – Proxies \nSection 15 – Rules Relating to Over-the-Counter Markets  \nSection 20A – Liability to Contemporaneous Traders for Insider Trading \nSection 21A – Civil Penalties for Insider Trading \n10b-1 – Prohibition of Use of Manipulative or Deceptive Devices or Contrivances with Respect to Certain \nSecurities Exempted from Registration \n10b-3 – Employment of Manipulative and Deceptive Devices by Brokers or Dealers \n10b-5 – Employment of Manipulative and Deceptive Devices \n10b5-1 – Trading on Material Nonpublic Information in Insider Trading Cases \n10b5-2 – Duties of Trust or Confidence in Misappropriation Insider Trading Cases \n10b-10 – Confirmation of Transactions \n15c1-2 – Fraud and Misrepresentation \n15c1-3 – Misrepresentation by Brokers, Dealers and Municipal Securities Dealers as to Registration \n15c2-12 – Municipal Securities Disclosure \n15l-1 – Regulation Best Interest \n17a-3 – Records to be Made by Certain Exchange Members, Brokers and Dealers \n17a-4 – Records to be Preserved by Certain Exchange Members, Brokers and Dealers \n17a-14 – Form CRS, for Preparation, Filing and Delivery of Form CRS \n\nInvestment Company Act of 1940 \n17a-6 – Exemption for Transactions with Portfolio Affiliates \n17a-7 – Exemption of Certain Purchase or Sale Transactions Between an Investment Company and Certain \nAffiliated Persons Thereof \n \n\nInsider Trading & Securities Fraud Enforcement Act of 1988 (ITSFEA) \n \nFederal Reserve Board  \n\nRegulation T \n \nFederal Trade Commission  \n\nTelemarketing Sales Rule \n \nUSA PATRIOT Act \n\nSection 314 – Cooperative Efforts to Deter Money Laundering \nSection 326 – Verification of Identification \nSection 352 – Anti-Money Laundering Programs  \n\n13 \n\n\n\n  \n\nSection 4: Overview of the Regulatory Framework \n \n4.1 SRO Regulatory Requirements for Associated Persons \n \n4.1.1  Registration and Continuing Education \n\n• SRO qualification and registration requirements \n◦ Definition of registered vs. non-registered person \n◦ Permitted activities of registered and non-registered persons \n◦ Ineligibility for membership or association \n◦ Background checks \n◦ Fingerprinting \n◦ Statutory disqualification \n◦ Failing to register an associated person \n\n• State registration requirements (e.g., blue-sky laws) \n• Continuing Education (CE) requirement  \n\n◦ Firm Element \n◦ Regulatory Element \n\n \n4.2 Employee Conduct and Reportable Events \n \n4.2.1  Employee Conduct  \n\n• Form U4 and Form U5 (e.g., purpose, when to update forms) \n• Consequences of filing misleading information or omitting information \n• Customer complaints \n• Potential red flags \n\n \n4.2.2 Reportable Events \n\n• Outside business activities  \n• Private securities transactions \n• Reporting of political contributions and consequences for exceeding dollar contribution thresholds  \n• Dollar and value limits for gifts and gratuities and non-cash compensation \n• Business entertainment  \n• Felony, financial-related misdemeanors, liens, bankruptcy  \n\n \nRules \n \n\nFINRA By-Laws \nArticle I – Definitions \n\n Article III – Qualifications of Members and Associated Persons \nArticle IV – Membership \nArticle V – Registered Representatives and Associated Persons \nArticle VI – Dues, Assessments, and Other Charges \nArticle XII – Disciplinary Proceedings   \nArticle XV – Limitations of Power \n\n \nFINRA Rules \n 0100 Series – General Standards \n\n1000 Series – Member Application and Associated Person Registration \n 1122 – Filing of Misleading Information as to Membership or Registration \n 1240 – Continuing Education \n\n2060 – Use of Information Obtained in Fiduciary Capacity \n 2263 – Arbitration Disclosure to Associated Persons Signing or Acknowledging Form U4 \n\n14 \n\n\n\n  \n2267 – Investor Education and Protection \n2310(c) – Non-cash Compensation \n2320(g)(4) – Non-cash Compensation \n2341(l)(5) – Non-cash Compensation \n3110(e) – Responsibility of Member to Investigate Applicants for Registration \n3220 – Influencing or Rewarding the Employees of Others \n3270 – Outside Business Activities of Registered Persons  \n3280 – Private Securities Transactions of an Associated Person  \n4513 – Written Customer Complaints  \n\n 4330 – Customer Protection – Permissible Use of Customers' Securities \n 4530 – Reporting Requirements \n 5110(h) – Non-cash Compensation \n 8312 – FINRA’s BrokerCheck Disclosure \n\n \nCBOE Rule \n\n7.10 – Fingerprint-based Background Checks of Exchange Directors, Officers, Employees and Others \n \nMSRB Rules  \n\nG-2 – Standards of Professional Qualifications  \nG-3 – Professional Qualification Requirements \nG-7 – Information Concerning Associated Persons  \nG-10 – Delivery of Investment Brochure  \nG-20 – Gifts, Gratuities and Non-cash Compensations  \nG-37 – Political Contributions and Prohibitions on Municipal Securities Business \n\n \nSEC Rules and Regulations \n\nSecurities Exchange Act of 1934 \nSection 3(a)(39) – Definitions and Application of Title (Statutory Disqualification) \n17f-2 – Fingerprinting of Securities Industry Personnel \n\n15",
  "job_functions": [
   {
    "key": "km",
    "section_num": 1,
    "name": "Knowledge of Capital Markets",
    "pct": 16,
    "questions": 12,
    "topics": [
     {
      "number": "1.1",
      "title": "Regulatory Entities, Agencies and Market Participants",
      "bullets": [],
      "subtopics": [
       {
        "number": "1.1.1",
        "title": "The Securities and Exchange Commission (SEC)",
        "bullets": [
         "The high-level purpose and mission of securities regulation",
         "Definition, jurisdiction and authority of the SEC"
        ]
       },
       {
        "number": "1.1.2",
        "title": "Self-regulatory Organizations (SROs)",
        "bullets": [
         "Purpose and mission of an SRO",
         "Jurisdiction and authority of SROs (e.g., CBOE, FINRA, MSRB)"
        ]
       },
       {
        "number": "1.1.3",
        "title": "Other Regulators and Agencies",
        "bullets": [
         "Department of the Treasury/IRS",
         "State regulators (e.g., NASAA)",
         "The Federal Reserve",
         "Securities Investor Protection Corporation (SIPC)",
         "Federal Deposit Insurance Corporation (FDIC)"
        ]
       },
       {
        "number": "1.1.4",
        "title": "Market Participants and their Roles",
        "bullets": [
         "Investors (e.g., accredited, institutional, retail)",
         "Broker-Dealers (e.g., introducing, clearing, prime brokers)",
         "Investment advisers",
         "Municipal advisors",
         "Issuers and underwriters",
         "Traders and market makers",
         "Custodians and trustees",
         "Transfer agents",
         "Depositories and clearing corporations (e.g., Depository Trust & Clearing Corporation (DTCC), Options Clearing / Corporation (OCC))"
        ]
       }
      ],
      "units": [
       1,
       9
      ]
     },
     {
      "number": "1.2",
      "title": "Market Structure",
      "bullets": [],
      "subtopics": [
       {
        "number": "1.2.1",
        "title": "Types of Markets",
        "bullets": [
         "The primary market",
         "The secondary market (e.g., electronic, over-the-counter (OTC), physical)",
         "The third market",
         "The fourth market"
        ]
       }
      ],
      "units": [
       1,
       9
      ]
     },
     {
      "number": "1.3",
      "title": "Economic Factors",
      "bullets": [],
      "subtopics": [
       {
        "number": "1.3.1",
        "title": "The Federal Reserve Board’s Impact on Business Activity and Market Stability",
        "bullets": [
         "Monetary vs. fiscal policy",
         "Open market activities and impact on economy",
         "Different rates (e.g., interest rate, discount rate, federal funds rate)"
        ]
       },
       {
        "number": "1.3.2",
        "title": "Business Economic Factors",
        "bullets": [
         "Purpose of financial statements (e.g., balance sheet, income statement)",
         "Business cycle (e.g., contraction, trough, expansion, peak)",
         "Indicators (e.g., leading, lagging, coincident, inflation)",
         "Basic effects on bond and equity markets (e.g., cyclical, defensive, growth)",
         "Principal economic theories (e.g., Keynesian, Monetarist)"
        ]
       },
       {
        "number": "1.3.3",
        "title": "International Economic Factors",
        "bullets": [
         "U.S. balance of payments",
         "Gross domestic product (GDP), gross national product (GNP)",
         "Exchange rates"
        ]
       }
      ],
      "units": [
       1,
       9
      ]
     },
     {
      "number": "1.4",
      "title": "Offerings",
      "bullets": [
       "Roles of participants (e.g., investment bankers, underwriting syndicate, municipal advisors)",
       "Types of offerings /  Public vs. private securities offering /  Initial public offering (IPO), secondary offering and follow-on offering /  Methods of distribution (e.g., best efforts, firm commitment)",
       "Shelf registrations and distributions (e.g., definition, purpose)",
       "Types and purpose of offering documents and delivery requirements (e.g., official statement, program disclosure / document, prospectus)",
       "Regulatory filing requirements and exemptions (e.g., SEC, blue-sky laws)"
      ],
      "subtopics": [],
      "units": [
       1,
       9
      ]
     }
    ],
    "rules": {
     "finra": [
      {
       "ref": "2266",
       "title": "SIPC Information",
       "act": null
      },
      {
       "ref": "2269",
       "title": "Disclosure of Participation or Interest in Primary or Secondary Distribution",
       "act": null
      },
      {
       "ref": "5250",
       "title": "Payments for Market Making",
       "act": null
      }
     ],
     "msrb": [
      {
       "ref": "G-11",
       "title": "Primary Offering Practices",
       "act": null
      },
      {
       "ref": "G-32",
       "title": "Disclosures in Connection with Primary Offerings",
       "act": null
      },
      {
       "ref": "G-34",
       "title": "CUSIP Numbers, New Issue and Market Information Requirements",
       "act": null
      }
     ],
     "cboe": [],
     "sec": [
      {
       "ref": "Section 7",
       "title": "Information Required in a Registration Statement",
       "act": "Securities Act of 1933"
      },
      {
       "ref": "Section 8",
       "title": "Taking Effect of Registration Statements and Amendments Thereto",
       "act": "Securities Act of 1933"
      },
      {
       "ref": "Section 10",
       "title": "Information Required in Prospectus",
       "act": "Securities Act of 1933"
      },
      {
       "ref": "Section 23",
       "title": "Unlawful Representations",
       "act": "Securities Act of 1933"
      },
      {
       "ref": "215",
       "title": "Accredited Investor",
       "act": "Securities Act of 1933"
      },
      {
       "ref": "431",
       "title": "Summary Prospectuses",
       "act": "Securities Act of 1933"
      },
      {
       "ref": "Section 3(a)",
       "title": "Definitions and Application of Title",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "Section 12",
       "title": "Registration Requirements for Securities",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "Section 15",
       "title": "Registration and Regulation of Brokers and Dealers",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "Section 15A",
       "title": "Registered Securities Associations",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "144",
       "title": "Persons Deemed Not to Be Engaged in a Distribution and Therefore Not Underwriters",
       "act": "Securities Act of 1933"
      },
      {
       "ref": "144A",
       "title": "Private Resales of Securities to Institutions",
       "act": "Securities Act of 1933"
      },
      {
       "ref": "145",
       "title": "Reclassification of Securities, Mergers, Consolidations and Acquisitions of Assets",
       "act": "Securities Act of 1933"
      },
      {
       "ref": "147",
       "title": "\"Part of an Issue,\" \"Person Resident,\" and \"Doing Business Within\" for Purposes of Section 3(a)(11)",
       "act": "Securities Act of 1933"
      },
      {
       "ref": "164",
       "title": "Post-filing Free Writing Prospectuses in Connection with Certain Registered Offerings",
       "act": "Securities Act of 1933"
      }
     ]
    }
   },
   {
    "key": "pr",
    "section_num": 2,
    "name": "Understanding Products & Their Risks",
    "pct": 44,
    "questions": 33,
    "topics": [
     {
      "number": "2.1",
      "title": "Products",
      "bullets": [],
      "subtopics": [
       {
        "number": "2.1.1",
        "title": "Equity Securities",
        "bullets": [
         "Types of equities /  Common stock /  Preferred stock /  Rights /  Warrants /  American Depositary Receipts (ADRs) / Knowledge of:",
         "Ownership (e.g., order of liquidation, limited liability)",
         "Voting rights",
         "Convertible",
         "Control and restrictions (e.g., SEC Rule 144)"
        ]
       },
       {
        "number": "2.1.2",
        "title": "Debt Instruments",
        "bullets": [
         "Treasury securities (e.g., bills, notes, receipts, bonds)",
         "Agency (e.g., asset-backed and mortgage-backed securities)",
         "Corporate bonds",
         "Municipal securities /  General obligation (GO) bonds /  Revenue bonds /  Others (e.g., special type bonds, taxable municipal securities, short-term obligations)",
         "Others (e.g., money market instruments, certificate of deposit (CD), bankers’ acceptance, commercial paper) / Knowledge of:",
         "Varying maturities",
         "Generate income (e.g., interest)",
         "Coupon value",
         "Par value",
         "Yield",
         "Ratings and rating agencies",
         "Callable and convertible features",
         "Short-term vs. long-term characteristics",
         "Relationship between price and interest rate",
         "Negotiated vs. competitive offerings via underwriters and syndicates",
         "Auction"
        ]
       },
       {
        "number": "2.1.3",
        "title": "Options",
        "bullets": [
         "Types of options /  Puts and calls /  Equity vs. index / Knowledge of:",
         "Hedging or speculation",
         "Expiration date",
         "Strike price",
         "Premium",
         "Underlying or cash settlement",
         "In-the-money, out-of-the money",
         "Covered vs. uncovered",
         "American vs. European",
         "Exercise and assignment",
         "Varying strategies (e.g., long, short)",
         "Special disclosures (e.g., Options Disclosure Document (ODD))",
         "Options Clearing Corporation (OCC) for listed options"
        ]
       },
       {
        "number": "2.1.4",
        "title": "Packaged Products",
        "bullets": [
         "Investment companies /  Types of investment companies / − Closed-end funds / − Open-end funds / − Unit investment trusts (UITs) / − Variable contracts/annuities / Knowledge of:",
         "Loads",
         "Share classes",
         "Net asset value (NAV)",
         "Disclosures",
         "Costs and fees",
         "Breakpoints",
         "Right of accumulation (ROA)",
         "Letter of intent (LOI)",
         "Net transactions",
         "Surrender charges",
         "Sales charges"
        ]
       },
       {
        "number": "2.1.5",
        "title": "Municipal Fund Securities",
        "bullets": [
         "529 Plans /  Prepaid tuition /  Savings plans",
         "Local government investment pools (LGIPs)",
         "ABLE accounts / Knowledge of:",
         "Municipal fund securities",
         "Owner vs. beneficiary",
         "Restricted use of plan assets",
         "Tax advantages",
         "Direct or adviser sold"
        ]
       },
       {
        "number": "2.1.6",
        "title": "Direct Participation Programs (DPPs)",
        "bullets": [
         "Types of DPPs /  Limited partnerships /  Tenants in common (TIC) / Knowledge of:",
         "Pass-through tax treatment",
         "Unlisted",
         "Generally illiquid"
        ]
       },
       {
        "number": "2.1.7",
        "title": "Real Estate Investment Trusts (REITs)",
        "bullets": [
         "Types of REITs /  Private /  Registered, non-listed /  Listed / Knowledge of:",
         "Real estate equity or debt",
         "Tax-advantaged income without double taxation"
        ]
       },
       {
        "number": "2.1.8",
        "title": "Hedge Funds",
        "bullets": [
         "Minimum investment",
         "Partnership structure",
         "Private equity",
         "Generally illiquid"
        ]
       },
       {
        "number": "2.1.9",
        "title": "Exchange-traded Products (ETPs)",
        "bullets": [
         "Types of ETPs /  Exchange-traded funds (ETFs) /  Exchange-traded notes (ETNs) / Knowledge of:",
         "Alternative investments to mutual funds",
         "Fee considerations",
         "Active vs. passive"
        ]
       }
      ],
      "units": [
       2,
       3,
       4,
       5
      ]
     },
     {
      "number": "2.2",
      "title": "Investment Risks",
      "bullets": [
       "Definition and Identification of Risk Types /  Capital /  Credit /  Currency /  Inflationary/purchasing power /  Interest rate/reinvestment /  Liquidity /  Market/systematic /  Non-systematic /  Political /  Prepayment",
       "Strategies for Mitigation of Risk /  Diversification /  Portfolio rebalancing /  Hedging"
      ],
      "subtopics": [],
      "units": [
       2,
       3,
       4,
       5
      ]
     }
    ],
    "rules": {
     "finra": [
      {
       "ref": "2261",
       "title": "Disclosure of Financial Condition",
       "act": null
      },
      {
       "ref": "2262",
       "title": "Disclosure of Financial Relationship with Issuer",
       "act": null
      },
      {
       "ref": "2310",
       "title": "Direct Participation Programs",
       "act": null
      },
      {
       "ref": "2330",
       "title": "Members’ Responsibilities Regarding Deferred Variable Annuities",
       "act": null
      },
      {
       "ref": "2342",
       "title": "“Breakpoint” Sales",
       "act": null
      },
      {
       "ref": "2360",
       "title": "Options",
       "act": null
      }
     ],
     "msrb": [
      {
       "ref": "D-12",
       "title": "Definition of Municipal Fund Securities",
       "act": null
      },
      {
       "ref": "G-17",
       "title": "Conduct of Municipal Securities and Municipal Advisory Activities",
       "act": null
      },
      {
       "ref": "G-30",
       "title": "Pricing and Commissions",
       "act": null
      },
      {
       "ref": "G-45",
       "title": "Reporting of Information on Municipal Fund Securities",
       "act": null
      }
     ],
     "cboe": [
      {
       "ref": "1.1",
       "title": "Definitions",
       "act": null
      }
     ],
     "sec": [
      {
       "ref": "3a11-1",
       "title": "Definition of the Term \"Equity Security\"",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "10b-18",
       "title": "Purchases of Certain Equity Securities by the Issuer and Others",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "Section 3(a)",
       "title": "Definitions - “Investment Company”",
       "act": "Investment Company Act of 1940"
      },
      {
       "ref": "Section 4",
       "title": "Classification of Investment Companies",
       "act": "Investment Company Act of 1940"
      },
      {
       "ref": "Section 5",
       "title": "Subclassification of Management Companies",
       "act": "Investment Company Act of 1940"
      },
      {
       "ref": "12b-1",
       "title": "Distribution of Shares by Registered Open-end Management Investment Company",
       "act": "Investment Company Act of 1940"
      }
     ]
    }
   },
   {
    "key": "tc",
    "section_num": 3,
    "name": "Trading, Customer Accounts & Prohibited Activities",
    "pct": 31,
    "questions": 23,
    "topics": [
     {
      "number": "3.1",
      "title": "Trading, Settlement and Corporate Actions",
      "bullets": [],
      "subtopics": [
       {
        "number": "3.1.1",
        "title": "Orders and Strategies",
        "bullets": [
         "Types of orders (e.g., market, stop, limit, good-til-canceled (GTC), discretionary vs. non-discretionary, solicited / vs. unsolicited)",
         "Buy and sell, bid-ask",
         "Trade capacity (e.g., principal, agency)",
         "Long and short, naked and covered",
         "Bearish and bullish"
        ]
       },
       {
        "number": "3.1.2",
        "title": "Investment Returns",
        "bullets": [
         "Components of return (e.g., interest, dividends, realized/unrealized gains, return on capital)",
         "Different types of dividends (e.g., cash, stock)",
         "Dividend payment dates (e.g., record date, ex-dividend date, payable date)",
         "Concepts of measurement (e.g., yield, yield to maturity (YTM), yield to call (YTC), total return, basis points)",
         "Cost basis requirements",
         "Benchmarks and indices"
        ]
       },
       {
        "number": "3.1.3",
        "title": "Trade Settlement",
        "bullets": [
         "Settlement time frames for various products (e.g., T, T + 1)",
         "Physical vs. book entry (e.g., delivery and settlement)"
        ]
       },
       {
        "number": "3.1.4",
        "title": "Corporate Actions",
        "bullets": [
         "Types of corporate actions (e.g., splits, reverse splits, buybacks, tender offers, exchange offers, rights offerings, / mergers and acquisitions (M&A))",
         "Impact of stock splits and reverse stock splits on market price and cost basis",
         "Adjustments to securities subject to corporate actions",
         "Delivery of notices and corporate action deadlines",
         "Proxies and proxy voting"
        ]
       }
      ],
      "units": [
       6,
       7,
       8,
       11
      ]
     },
     {
      "number": "3.2",
      "title": "Customer Accounts and Compliance Considerations",
      "bullets": [],
      "subtopics": [
       {
        "number": "3.2.1",
        "title": "Account Types and Characteristics",
        "bullets": [
         "Cash",
         "Margin",
         "Options",
         "Discretionary vs. non-discretionary",
         "Fee-based vs. commission",
         "Educational accounts"
        ]
       },
       {
        "number": "3.2.2",
        "title": "Customer Account Registrations",
        "bullets": [
         "Individual",
         "Joint",
         "Corporate/institutional",
         "Trust (e.g., revocable, irrevocable)",
         "Custodial (e.g., UTMA)",
         "Partnerships",
         "Retirement (e.g., individual retirement account (IRA), qualified plans) / ◦ Types and characteristics / ◦ Required minimum distributions / ◦ Contributions"
        ]
       },
       {
        "number": "3.2.3",
        "title": "Anti-money Laundering (AML)",
        "bullets": [
         "Definition of money laundering",
         "Stages of money laundering (e.g., structuring, layering, placement)",
         "AML compliance program",
         "Suspicious Activity Report (SAR)",
         "Currency Transaction Report (CTR)",
         "FinCEN",
         "Office of Foreign Asset Control (OFAC) and the Specially Designated Nationals and Blocked Persons (SDNs) / List"
        ]
       },
       {
        "number": "3.2.4",
        "title": "Books and Records and Privacy Requirements",
        "bullets": [
         "Books and records retention requirements",
         "Confirmations and account statements",
         "Holding of customer mail",
         "Business continuity plans (BCP)",
         "Customer protection and custody of assets",
         "Privacy requirements (e.g., Regulation S-P) / ◦ Nonpublic personal information / ◦ Confidentiality of information / ◦ Privacy notifications / ◦ Safeguard requirements"
        ]
       },
       {
        "number": "3.2.5",
        "title": "Communications with the Public and General Best Interest Obligations and Suitability Requirements",
        "bullets": [
         "Communications with the public and telemarketing / ◦ Classifications and general requirements / ◦ Do-not-call list",
         "Best interest obligations and suitability requirements / ◦ Know-your-customer (KYC) / ◦ General requirements (e.g., what constitutes a recommendation)"
        ]
       }
      ],
      "units": [
       6,
       7,
       8,
       11
      ]
     },
     {
      "number": "3.3",
      "title": "Prohibited Activities",
      "bullets": [],
      "subtopics": [
       {
        "number": "3.3.1",
        "title": "Market Manipulation",
        "bullets": [
         "Definition of market manipulation",
         "Types of market manipulation (e.g., market rumors, pump and dump, front running, excessive trading, marking / the close, marking the open, backing away, freeriding)"
        ]
       },
       {
        "number": "3.3.2",
        "title": "Insider Trading",
        "bullets": [
         "Definition of insider trading",
         "Definition of material nonpublic information",
         "Identifying involved parties",
         "Penalties (e.g., fines, expulsion, incarceration)"
        ]
       },
       {
        "number": "3.3.3",
        "title": "Other Prohibited Activities",
        "bullets": [
         "Restrictions preventing associated persons from purchasing initial public offerings (IPOs)",
         "Use of manipulative, deceptive or other fraudulent devices",
         "Improper use of customers’ securities or funds / ◦ Borrowing from customers / ◦ Sharing in customer accounts",
         "Financial exploitation of seniors",
         "Activities of unregistered persons / ◦ Prohibition against paying commissions to unregistered persons / ◦ Prohibition against solicitation of customers and taking orders",
         "Falsifying or withholding documents / ◦ Signatures of convenience / ◦ Responding to regulatory requests",
         "Prohibited activities related to maintenance of books and records (e.g., falsifying records and improper / maintenance/retention of records)"
        ]
       }
      ],
      "units": [
       6,
       7,
       8,
       11
      ]
     }
    ],
    "rules": {
     "finra": [
      {
       "ref": "2010",
       "title": "Standards of Commercial Honor and Principles of Trade",
       "act": null
      },
      {
       "ref": "2020",
       "title": "Use of Manipulative, Deceptive or Other Fraudulent Devices",
       "act": null
      },
      {
       "ref": "2040",
       "title": "Payments to Unregistered Persons",
       "act": null
      },
      {
       "ref": "2090",
       "title": "Know Your Customer",
       "act": null
      },
      {
       "ref": "2111",
       "title": "Suitability",
       "act": null
      },
      {
       "ref": "2120",
       "title": "Commissions, Mark Ups and Charges",
       "act": null
      },
      {
       "ref": "2150",
       "title": "Improper Use of Customers’ Securities or Funds; Prohibition Against Guarantees and Sharing in Accounts",
       "act": null
      },
      {
       "ref": "2165",
       "title": "Financial Exploitation of Specified Adults",
       "act": null
      },
      {
       "ref": "2210",
       "title": "Communications with the Public",
       "act": null
      },
      {
       "ref": "2231",
       "title": "Customer Account Statements",
       "act": null
      },
      {
       "ref": "2251",
       "title": "Forwarding of Proxy and Other Issuer-related Materials",
       "act": null
      },
      {
       "ref": "2264",
       "title": "Margin Disclosure Statement",
       "act": null
      },
      {
       "ref": "2232",
       "title": "Customer Confirmations",
       "act": null
      },
      {
       "ref": "3150",
       "title": "Holding of Customer Mail",
       "act": null
      },
      {
       "ref": "3210",
       "title": "Accounts at Other Broker-Dealers and Financial Institutions",
       "act": null
      },
      {
       "ref": "3230",
       "title": "Telemarketing",
       "act": null
      },
      {
       "ref": "3240",
       "title": "Borrowing from or Lending to Customers",
       "act": null
      },
      {
       "ref": "3250",
       "title": "Designation of Accounts",
       "act": null
      },
      {
       "ref": "3260",
       "title": "Discretionary Accounts",
       "act": null
      },
      {
       "ref": "3310",
       "title": "Anti-money Laundering Compliance Program",
       "act": null
      },
      {
       "ref": "4210",
       "title": "Margin Requirements",
       "act": null
      },
      {
       "ref": "4370",
       "title": "Business Continuity Plans and Emergency Contact Information",
       "act": null
      },
      {
       "ref": "4511",
       "title": "General Requirements",
       "act": null
      },
      {
       "ref": "4512",
       "title": "Customer Account Information",
       "act": null
      },
      {
       "ref": "4514",
       "title": "Authorization Records for Negotiable Instruments Drawn From a Customer's Account",
       "act": null
      },
      {
       "ref": "5130",
       "title": "Restrictions on the Purchase and Sale of Initial Equity Public Offerings",
       "act": null
      },
      {
       "ref": "5210",
       "title": "Publication of Transactions and Quotations",
       "act": null
      },
      {
       "ref": "5220",
       "title": "Offers at Stated Prices",
       "act": null
      },
      {
       "ref": "5230",
       "title": "Payments Involving Publications that Influence the Market Price of a Security",
       "act": null
      },
      {
       "ref": "5240",
       "title": "Anti-intimidation/Coordination",
       "act": null
      },
      {
       "ref": "5270",
       "title": "Front Running of Block Transactions",
       "act": null
      },
      {
       "ref": "5280",
       "title": "Trading Ahead of Research Reports",
       "act": null
      },
      {
       "ref": "5290",
       "title": "Order Entry and Execution Practices",
       "act": null
      },
      {
       "ref": "5310",
       "title": "Best Execution and Interpositioning",
       "act": null
      },
      {
       "ref": "5320",
       "title": "Prohibition Against Trading Ahead of Customer Orders",
       "act": null
      },
      {
       "ref": "6438",
       "title": "Displaying Priced Quotations in Multiple Quotation Mediums",
       "act": null
      }
     ],
     "msrb": [
      {
       "ref": "G-8",
       "title": "Books and Records to be Made by Brokers, Dealers, Municipal Securities Dealers, and Municipal Advisors",
       "act": null
      },
      {
       "ref": "G-9",
       "title": "Preservation of Records",
       "act": null
      },
      {
       "ref": "G-13",
       "title": "Quotations",
       "act": null
      },
      {
       "ref": "G-14",
       "title": "Reports of Sales or Purchases",
       "act": null
      },
      {
       "ref": "G-15",
       "title": "Confirmation, Clearance, Settlement and Other Uniform Practice Requirements with Respect to Transactions",
       "act": null
      },
      {
       "ref": "G-18",
       "title": "Best Execution",
       "act": null
      },
      {
       "ref": "G-21",
       "title": "Advertising",
       "act": null
      },
      {
       "ref": "G-25",
       "title": "Improper Use of Assets",
       "act": null
      },
      {
       "ref": "G-39",
       "title": "Telemarketing",
       "act": null
      },
      {
       "ref": "G-41",
       "title": "Anti-money Laundering Compliance Program",
       "act": null
      },
      {
       "ref": "G-47",
       "title": "Time of Trade Disclosure",
       "act": null
      }
     ],
     "cboe": [],
     "sec": [
      {
       "ref": "Section 10",
       "title": "Regulation of the Use of Manipulative and Deceptive Devices",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "Section 11(d)",
       "title": "Trading by Members of Exchanges, Brokers and Dealers – “Prohibition on Extension of Credit by",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "Section 14",
       "title": "Proxies",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "Section 15",
       "title": "Rules Relating to Over-the-Counter Markets",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "Section 20A",
       "title": "Liability to Contemporaneous Traders for Insider Trading",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "Section 21A",
       "title": "Civil Penalties for Insider Trading",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "10b-1",
       "title": "Prohibition of Use of Manipulative or Deceptive Devices or Contrivances with Respect to Certain",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "10b-3",
       "title": "Employment of Manipulative and Deceptive Devices by Brokers or Dealers",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "10b-5",
       "title": "Employment of Manipulative and Deceptive Devices",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "10b5-1",
       "title": "Trading on Material Nonpublic Information in Insider Trading Cases",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "10b5-2",
       "title": "Duties of Trust or Confidence in Misappropriation Insider Trading Cases",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "10b-10",
       "title": "Confirmation of Transactions",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "15c1-2",
       "title": "Fraud and Misrepresentation",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "15c1-3",
       "title": "Misrepresentation by Brokers, Dealers and Municipal Securities Dealers as to Registration",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "15c2-12",
       "title": "Municipal Securities Disclosure",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "15l-1",
       "title": "Regulation Best Interest",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "17a-3",
       "title": "Records to be Made by Certain Exchange Members, Brokers and Dealers",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "17a-4",
       "title": "Records to be Preserved by Certain Exchange Members, Brokers and Dealers",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "17a-14",
       "title": "Form CRS, for Preparation, Filing and Delivery of Form CRS",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "17a-6",
       "title": "Exemption for Transactions with Portfolio Affiliates",
       "act": "Investment Company Act of 1940"
      },
      {
       "ref": "17a-7",
       "title": "Exemption of Certain Purchase or Sale Transactions Between an Investment Company and Certain",
       "act": "Investment Company Act of 1940"
      },
      {
       "ref": "Section 314",
       "title": "Cooperative Efforts to Deter Money Laundering",
       "act": "Investment Company Act of 1940"
      },
      {
       "ref": "Section 326",
       "title": "Verification of Identification",
       "act": "Investment Company Act of 1940"
      },
      {
       "ref": "Section 352",
       "title": "Anti-Money Laundering Programs",
       "act": "Investment Company Act of 1940"
      }
     ]
    }
   },
   {
    "key": "rf",
    "section_num": 4,
    "name": "Overview of the Regulatory Framework",
    "pct": 9,
    "questions": 7,
    "topics": [
     {
      "number": "4.1",
      "title": "SRO Regulatory Requirements for Associated Persons",
      "bullets": [],
      "subtopics": [
       {
        "number": "4.1.1",
        "title": "Registration and Continuing Education",
        "bullets": [
         "SRO qualification and registration requirements / ◦ Definition of registered vs. non-registered person / ◦ Permitted activities of registered and non-registered persons / ◦ Ineligibility for membership or association / ◦ Background checks / ◦ Fingerprinting / ◦ Statutory disqualification / ◦ Failing to register an associated person",
         "State registration requirements (e.g., blue-sky laws)",
         "Continuing Education (CE) requirement / ◦ Firm Element / ◦ Regulatory Element"
        ]
       }
      ],
      "units": [
       10,
       12
      ]
     },
     {
      "number": "4.2",
      "title": "Employee Conduct and Reportable Events",
      "bullets": [],
      "subtopics": [
       {
        "number": "4.2.1",
        "title": "Employee Conduct",
        "bullets": [
         "Form U4 and Form U5 (e.g., purpose, when to update forms)",
         "Consequences of filing misleading information or omitting information",
         "Customer complaints",
         "Potential red flags"
        ]
       },
       {
        "number": "4.2.2",
        "title": "Reportable Events",
        "bullets": [
         "Outside business activities",
         "Private securities transactions",
         "Reporting of political contributions and consequences for exceeding dollar contribution thresholds",
         "Dollar and value limits for gifts and gratuities and non-cash compensation",
         "Business entertainment",
         "Felony, financial-related misdemeanors, liens, bankruptcy"
        ]
       }
      ],
      "units": [
       10,
       12
      ]
     }
    ],
    "rules": {
     "finra": [
      {
       "ref": "1122",
       "title": "Filing of Misleading Information as to Membership or Registration",
       "act": null
      },
      {
       "ref": "1240",
       "title": "Continuing Education",
       "act": null
      },
      {
       "ref": "2060",
       "title": "Use of Information Obtained in Fiduciary Capacity",
       "act": null
      },
      {
       "ref": "2263",
       "title": "Arbitration Disclosure to Associated Persons Signing or Acknowledging Form U4",
       "act": null
      },
      {
       "ref": "2267",
       "title": "Investor Education and Protection",
       "act": null
      },
      {
       "ref": "2310(c)",
       "title": "Non-cash Compensation",
       "act": null
      },
      {
       "ref": "2320(g)(4)",
       "title": "Non-cash Compensation",
       "act": null
      },
      {
       "ref": "2341(l)(5)",
       "title": "Non-cash Compensation",
       "act": null
      },
      {
       "ref": "3110(e)",
       "title": "Responsibility of Member to Investigate Applicants for Registration",
       "act": null
      },
      {
       "ref": "3220",
       "title": "Influencing or Rewarding the Employees of Others",
       "act": null
      },
      {
       "ref": "3270",
       "title": "Outside Business Activities of Registered Persons",
       "act": null
      },
      {
       "ref": "3280",
       "title": "Private Securities Transactions of an Associated Person",
       "act": null
      },
      {
       "ref": "4513",
       "title": "Written Customer Complaints",
       "act": null
      },
      {
       "ref": "4330",
       "title": "Customer Protection – Permissible Use of Customers' Securities",
       "act": null
      },
      {
       "ref": "4530",
       "title": "Reporting Requirements",
       "act": null
      },
      {
       "ref": "5110(h)",
       "title": "Non-cash Compensation",
       "act": null
      },
      {
       "ref": "8312",
       "title": "FINRA’s BrokerCheck Disclosure",
       "act": null
      }
     ],
     "msrb": [
      {
       "ref": "G-2",
       "title": "Standards of Professional Qualifications",
       "act": null
      },
      {
       "ref": "G-3",
       "title": "Professional Qualification Requirements",
       "act": null
      },
      {
       "ref": "G-7",
       "title": "Information Concerning Associated Persons",
       "act": null
      },
      {
       "ref": "G-10",
       "title": "Delivery of Investment Brochure",
       "act": null
      },
      {
       "ref": "G-20",
       "title": "Gifts, Gratuities and Non-cash Compensations",
       "act": null
      },
      {
       "ref": "G-37",
       "title": "Political Contributions and Prohibitions on Municipal Securities Business",
       "act": null
      }
     ],
     "cboe": [
      {
       "ref": "7.10",
       "title": "Fingerprint-based Background Checks of Exchange Directors, Officers, Employees and Others",
       "act": null
      }
     ],
     "sec": [
      {
       "ref": "Section 3(a)(39)",
       "title": "Definitions and Application of Title (Statutory Disqualification)",
       "act": "Securities Exchange Act of 1934"
      },
      {
       "ref": "17f-2",
       "title": "Fingerprinting of Securities Industry Personnel",
       "act": "Securities Exchange Act of 1934"
      }
     ]
    }
   }
  ]
 },
 "units": {
  "1": {
   "num": 1,
   "name": "Primary & Secondary Markets",
   "jf": "km",
   "topics": [
    "1.1",
    "1.2",
    "1.3",
    "1.4"
   ]
  },
  "9": {
   "num": 9,
   "name": "Economics",
   "jf": "km",
   "topics": [
    "1.1",
    "1.2",
    "1.3",
    "1.4"
   ]
  },
  "2": {
   "num": 2,
   "name": "Equity Securities",
   "jf": "pr",
   "topics": [
    "2.1",
    "2.2"
   ]
  },
  "3": {
   "num": 3,
   "name": "Debt Securities",
   "jf": "pr",
   "topics": [
    "2.1",
    "2.2"
   ]
  },
  "4": {
   "num": 4,
   "name": "Investment Companies & Insurance",
   "jf": "pr",
   "topics": [
    "2.1",
    "2.2"
   ]
  },
  "5": {
   "num": 5,
   "name": "Other Investment Vehicles",
   "jf": "pr",
   "topics": [
    "2.1",
    "2.2"
   ]
  },
  "6": {
   "num": 6,
   "name": "Customer Accounts",
   "jf": "tc",
   "topics": [
    "3.1",
    "3.2",
    "3.3"
   ]
  },
  "7": {
   "num": 7,
   "name": "Risk & Recommendations",
   "jf": "tc",
   "topics": [
    "3.1",
    "3.2",
    "3.3"
   ]
  },
  "8": {
   "num": 8,
   "name": "Tax & Yields",
   "jf": "tc",
   "topics": [
    "3.1",
    "3.2",
    "3.3"
   ]
  },
  "11": {
   "num": 11,
   "name": "Regulatory Issues & Ethics",
   "jf": "tc",
   "topics": [
    "3.1",
    "3.2",
    "3.3"
   ]
  },
  "10": {
   "num": 10,
   "name": "Regulators",
   "jf": "rf",
   "topics": [
    "4.1",
    "4.2"
   ]
  },
  "12": {
   "num": 12,
   "name": "Joining & Leaving a Member Firm",
   "jf": "rf",
   "topics": [
    "4.1",
    "4.2"
   ]
  }
 },
 "glossary": {
  "source": "investor.gov",
  "license": "public-domain",
  "path": "corpus/Investor.gov_Glossary_Complete.txt"
 }
};
